...
int main(void)
{
	...
	cout<<strerror(errno);
	perror("error:");
	Value login,regist,changeinf,qunchat,addqun,saveHotTie;
	FastWriter writer;
	...
	saveHotTie["typecode"] = 2060;
	saveHotTie["username"] = "finder";
	saveHotTie["title"] = "pdgmp";
	...
	string strsavehottie = writer.write(saveHotTie);

	cout<<"1--regist   2--login   3--changeinf    4--qunchat	5--addqun 	6--savehottie"<<endl;
	while(1)
	{
		...
		switch(i)
		{
			...
			case 6:senddata(strsavehottie,client);break;
			default:break;
		}
	}
...
{	
	...
	*(short int *)data = htons(datalen);
	msg.copy(data+2,datalen,0);
	cout<<"sorppose data length :"<<datalen<<endl;
	cout<<"data length in data:"<<*(short int*)data<<endl;
	cout<<"send data:"<<(data+2)<<endl;
	int sendlen = send(client,data,datalen+2,0);
	cout<<"real data length:"<<sendlen<<endl;
	free(data);
}