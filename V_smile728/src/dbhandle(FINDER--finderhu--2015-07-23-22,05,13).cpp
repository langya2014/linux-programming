...
struct USER *dblogin(char name[32], char passwd[24], long socket)
{
	...
	lginres = new USER();
	mysql_init(&mysql);
	if(!mysql_real_connect(&mysql, "127.0.0.1", "root", "root123", "VSimle_db", 3306, 0, 0))
	{
...
struct recordlist *showchatrcd(string username, string qunname, int num)
{
	...
	while(row = mysql_fetch_row(res))
	{
		...
		cout<<"strcmp:"<<pname<<endl;
		//cout<<"aaabbbbb"<<endl;
		if(pname == 0)
		{
			qunID = (atoi(row[0])) % 10;
			cout<<"row[0]"<<atoi(row[0])<<endl;
			switch(qunID){
				case 1:
	...
	string comd = "select * from "+recordno+" order by id DESC";
	cout << comd.c_str() << endl;
	if(mysql_real_query(&mysql, comd.c_str(), (unsigned long)strlen(comd.c_str())))
	{
	...
	while(row2 = mysql_fetch_row(res2))
	{
		...
		if(i >= num)
		{
			...
			if(count == 10)
				break;
			//delete p_rcd;	
			/***** this can cause the server program down,so need to note! 
...
int addchatrecord(string username, string qunname, string msg, long *&skt, long &online_num)
{
	...
	while(row = mysql_fetch_row(res))
	{
		...
		if(pname == 0)
		{
			int qunID = atoi(row[0]);
			int modID = qunID % 10; 	//qunID mod 10,qu ge wei.
			//cout << qunID << endl;
			//cout << (int)(*qunID) << endl;
			cout << modID << endl;
			switch(modID){
...
map<string, int> query_grpmsg(string username)
{
	...
	while(row = mysql_fetch_row(res))
	{
		if(strcmp(row[1], username.c_str()) == 0)
		{
			...
			while(row2 = mysql_fetch_row(res2))
			{
				if(strcmp(row2[1], groupname.c_str()) == 0)
				{
					groupID = (atoi(row2[0])) % 10;
					string recordtab;
					switch(groupID)
					{
						case 0:
							recordtab = "qun0_record";
							break;
						case 1:
							recordtab = "qun1_record";
							break;
						case 2:
							recordtab = "qun2_record";
							break;
						case 3:
							recordtab = "qun3_record";
							break;
						case 4:
							recordtab = "qun4_record";
							break;
						case 5:
							recordtab = "qun5_record";
							break;
						case 6:
							recordtab = "qun6_record";
							break;
						case 7:
							recordtab = "qun7_record";
							break;
						case 8:
							recordtab = "qun8_record";
							break;
						case 9:
							recordtab = "qun9_record";
							break;
						default:
							cout << "exceed the maximun numberof the group, error!" << endl;
					...
					string cmd = "select * from "+recordtab+" order by id DESC";
					cout << cmd.c_str() << endl;
					if(mysql_real_query(&mysql, cmd.c_str(), (unsigned long)strlen(cmd.c_str())))
					{
						cout << "mysql_real_query failure!" <<endl;
...
struct recordlist *querynewrcd(string username, string groupname, int newmsgnum)
{
	...
	while(row = mysql_fetch_row(res))
	{
		if(strcmp(row[1], groupname.c_str()) == 0)
		{
			groupID = (atoi(row[0])) % 10;
			//string recordtab;
			switch(groupID)
			{
				case 0:
	...
	while(row2 = mysql_fetch_row(res2))
	{
		...
		cout<<"int dkdkdkfsdfasdfadf"<<endl;		
		string speaker;
		speaker = p_rcd->username;
